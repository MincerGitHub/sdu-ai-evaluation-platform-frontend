<template>
  <div class="teacher-all-applications-page">
    <div class="page-header">
      <h2>全部申报</h2>
      <div class="toolbar">
        <el-input v-model.trim="filters.keyword" placeholder="搜索申报名称" clearable style="width: 220px"
        ...
        <el-select v-model="filters.class_id" placeholder="班级" clearable style="width: 120px">
          <el-option v-for="item in classOptions" :key="item" :label="item" :value="item" />
        </el-select>
        <el-button type="primary" @click="fetchList(1)">筛选</el-button>
        <el-button @click="resetFilters">重置</el-button>
        <el-button type="danger" :disabled="selectedRows.length === 0" @click="handleBatchArchive">
          批量归档
        </el-button>
      </div>
    </div>

    <el-table :data="rows" border stripe v-loading="loading" empty-text="暂无申报数据" @selection-change="onSelectionChange">
      ...
      <el-table-column label="操作" width="170">
        <template #default="{ row }">
          <el-button link type="primary" :disabled="row.status === 'archived'" @click="openRecheckDialog(row)">
            审核申报
          </el-button>
    ...
    <div class="pagination-wrap">
      <el-pagination background layout="prev, pager, next, total" :current-page="pagination.page"
        :page-size="pagination.size" :total="pagination.total" @current-change="fetchList" />
    </div>

    <el-dialog v-model="recheckDialogVisible" title="审核申报" width="560px" @closed="resetDialogState">
      <div class="audit-dialog-body">
        ...
        <div class="audit-field">
          ...
          <div class="audit-value audit-value--multiline">
            <template v-if="attachmentList.length">
              <a v-for="(item, index) in attachmentList" :key="item.file_id || item.url || index"
                :href="getAttachmentUrl(item)" class="attachment-link" target="_blank" rel="noopener noreferrer">
        ...
        <div class="audit-field">
          ...
          <el-input v-model.trim="recheckForm.comment" maxlength="300" placeholder="默认空" show-word-limit />
        </div>
      </div>
      <template #footer>
        <div class="audit-footer">
          <el-button type="danger" :loading="rechecking" @click="submitRecheck('approved')">
            通过
          </el-button>
          <el-button type="danger" plain :loading="rechecking" @click="submitRecheck('rejected')">
            拒绝
          </el-button>
          <el-button link type="danger" :disabled="rechecking" @click="recheckDialogVisible = false">
            返回
          </el-button>
        </div>
      </template>
...
import applicationService from '@/services/statisticService'
